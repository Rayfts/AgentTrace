...
use agenttrace_protocol::{
    EventEnvelope, EventKind, HarnessId, IntegrationMode, Provenance, RawSource,
};
use futures::stream;
...
pub struct StructuredRunRegistry {
...
impl StructuredRunRegistry {
    pub async fn launch(
    ...
    ) -> Result<RunHandle, AdapterError> {
        {
            let runs = self
                .runs
                .lock()
                .map_err(|_| AdapterError::Protocol("run registry lock poisoned".into()))?;
        ...
        let mut process = SupervisedProcess::spawn(spec)
            .await
        ...
        tokio::spawn(async move {
            ...
            loop {
                tokio::select! {
                    _ = &mut cancel_rx, if !cancellation_seen => {
                        ...
                        match process_event {
                            ...
                            Ok(ProcessEvent::Stdout { bytes }) => {
                                ...
                                while let Some(newline) = stdout.iter().position(|byte| *byte == b'\n') {
                                    ...
                                    for event in normalizer(run_id, trace_id, &mut sequence, line.trim_end_matches('\r')) {
                            ...
                            Ok(ProcessEvent::Exited { code, success, duration }) => {
                                ...
                                let mut event = process_envelope(
                                    run_id,
                                    trace_id,
                                    &mut sequence,
                                    harness,
                            ...
                            Err(error) => {
                                let _ = event_tx
                                    .send(Err(AdapterError::Protocol(format!("process supervisor: {error}"))))
                                    .await;
                                break;
                            }
                        }
        ...
        Ok(RunHandle {
            run_id,
            harness,
            integration_mode: IntegrationMode::StructuredStream,
            native_id: None,
        })
    }
    ...
    pub fn events(&self, run_id: Uuid) -> Result<EventStream, AdapterError> {
        let receiver = self
            ...
            .map_err(|_| AdapterError::Protocol("run registry lock poisoned".into()))?
            .get_mut(&run_id)
            .ok_or(AdapterError::RunNotActive(run_id))?
            .events
            .take()
            .ok_or_else(|| AdapterError::InvalidRequest(format!("events for run {run_id} were already consumed")))?;
...
pub async fn detect_binary(
    binary: &str,
    integration_modes: Vec<IntegrationMode>,
    ...
    match Command::new(binary).arg("--version").output().await {
        Ok(output) => {
            ...
            Ok(Detection {
                ...
                executable: output.status.success().then(|| binary.into()),
                version: (!version.is_empty()).then_some(version),
                integration_modes,
                notes: Vec::new(),
            })
        }
        Err(error) if error.kind() == ErrorKind::NotFound => Ok(Detection {
            installed: false,
            executable: None,
            version: None,
            integration_modes,
...
pub fn native_harness_event(
    ...
    *sequence += 1;
    ...
    event.raw_source = Some(RawSource {
    ...
    *sequence += 1;
    EventEnvelope::new(
        ...
        *sequence,
        harness,
        IntegrationMode::StructuredStream,
        Provenance::native("agenttrace-process:stdout"),
