...
impl StructuredRunRegistry {
        ...
        let receiver = self
            ...
            .ok_or_else(|| {
                AdapterError::InvalidRequest(format!(
                    "events for run {run_id} were already consumed"
                ))
            })?;
    ...
    pub fn cancel(&self, run_id: Uuid) -> Result<(), AdapterError> {
        ...
        sender
            .send(())
            .map_err(|_| AdapterError::RunNotActive(run_id))
    }
}
...
pub fn native_harness_event_mode(
    run_id: Uuid,
    trace_id: Uuid,
    sequence: &mut u64,
    harness: HarnessId,
...
pub fn captured_stdout_event_mode(
    run_id: Uuid,
    trace_id: Uuid,
    sequence: &mut u64,
    harness: HarnessId,
