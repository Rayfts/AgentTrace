...
impl HarnessAdapter for OpenCodeAdapter {
    ...
    async fn capabilities(&self) -> Result<CapabilityReport, AdapterError> {
        ...
        capabilities.insert(
            ...
            CapabilityEvidence {
                level: ProvenanceLevel::Native,
                source: "text/reasoning and step-finish records".into(),
                notes: Some("full provider request bodies are not claimed".into()),
            },
        );
        capabilities.insert(
            ...
            CapabilityEvidence {
                level: ProvenanceLevel::Native,
                source: "tool records when MCP tools are surfaced by OpenCode".into(),
                notes: Some(
                    "MCP is classified only from explicit MCP tool identity/metadata".into(),
                ),
            },
        );
    ...
    async fn start(&self, request: RunRequest) -> Result<RunHandle, AdapterError> {
        ...
        spec.args = args;
        self.runs
            .launch(request.run_id, HarnessId::Opencode, spec, normalize_line)
            .await
    }
...
fn command(argv: &[OsString]) -> Result<(OsString, Vec<OsString>), AdapterError> {
    let Some((program, rest)) = argv.split_first() else {
        return Err(AdapterError::InvalidRequest(
            "missing OpenCode command".into(),
        ));
    };
    let mut args = rest.to_vec();
    if !args.iter().any(|arg| arg.to_string_lossy() == "run") {
        return Err(AdapterError::InvalidRequest(
            "structured OpenCode tracing requires `opencode run ...`".into(),
        ));
    }
    if args.iter().any(|arg| arg.to_string_lossy() == "--mini") {
        return Err(AdapterError::InvalidRequest(
            "OpenCode --mini is interactive and cannot be combined with JSON tracing".into(),
        ));
    }
...
fn ensure_pair(args: &mut Vec<OsString>, flag: &str, expected: &str) -> Result<(), AdapterError> {
    if let Some(index) = args.iter().position(|arg| arg.to_string_lossy() == flag) {
        let value = args
            ...
            .ok_or_else(|| AdapterError::InvalidRequest(format!("{flag} requires a value")))?;
        if value != expected {
            return Err(AdapterError::InvalidRequest(format!(
                "{flag} must be {expected} for tracing"
            )));
        }
    } else {
        args.push(flag.into());
...
pub fn normalize_line(
    ...
    match raw.get("type").and_then(Value::as_str).unwrap_or("unknown") {
        "text" => vec![native(
            ...
            raw.get("part").cloned().unwrap_or(Value::Null),
            raw,
        )],
        ...
        "step_start" => vec![native(
            run_id,
            trace_id,
            sequence,
            EventKind::Checkpoint,
            json!({"phase":"step_start","part":raw.get("part")}),
            raw,
        )],
        ...
        "error" => {
            ...
            let mut event = native(
                ...
                sequence,
                EventKind::Error,
                raw.get("error").cloned().unwrap_or(Value::Null),
                raw,
            );
...
fn normalize_step_finish(
    ...
    let usage = tokens.map(|v| TokenUsage {
        input_tokens: v.get("input").and_then(Value::as_u64),
        output_tokens: v.get("output").and_then(Value::as_u64),
        cached_input_tokens: v.pointer("/cache/read").and_then(Value::as_u64),
        cache_write_input_tokens: v.pointer("/cache/write").and_then(Value::as_u64),
...
fn normalize_tool(
    ...
    let mut events = vec![native(
        ...
        sequence,
        EventKind::ToolCall,
        json!({"call_id":part.get("callID"),"tool":tool,"input":input}),
        raw.clone(),
    )];
    let status = state
        .get("status")
    ...
    if matches!(status, "completed" | "error") {
        events.push(native(run_id, trace_id, sequence, EventKind::ToolResult, json!({"call_id":part.get("callID"),"tool":tool,"status":status,"output":state.get("output"),"error":state.get("error")}), raw.clone()));
    }
    match tool {
        "bash" => {
            let command = input.get("command").and_then(Value::as_str).unwrap_or("");
            let mut start = native(
                run_id,
                trace_id,
                sequence,
            ...
            start.command = Some(CommandInfo {
                program: "shell".into(),
        ...
        "edit" | "patch" | "apply_patch" => push_file(
            &mut events,
            run_id,
            trace_id,
            sequence,
        ...
        _ if tool.starts_with("mcp_") || tool.starts_with("mcp__") => events.push(native(
            run_id,
            trace_id,
            sequence,
...
fn push_file(
    ...
    if let Some(path) = path {
        if kind == EventKind::FileRead {
            impact.paths_read.push(path.into());
        } else {
            impact.paths_written.push(path.into());
        }
    }
...
mod tests {
    use super::*;
    #[test]
    fn fixture_maps_tool_and_step_usage() {
        let fixture = include_str!("../../../../fixtures/opencode/run.jsonl");
        let mut seq = 0;
        let r = Uuid::new_v4();
        let t = Uuid::new_v4();
        let events: Vec<_> = fixture
            .lines()
            .flat_map(|l| normalize_line(r, t, &mut seq, l))
            .collect();
        assert!(events.iter().any(|e| e.kind == EventKind::ShellCommand));
        assert!(events.iter().any(|e| e.kind == EventKind::ToolResult));
        assert!(events.iter().any(|e| e.kind == EventKind::ModelUsage));
    }
}
